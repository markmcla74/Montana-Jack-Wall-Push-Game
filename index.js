 ...
 //alert("If you see this, the script is loading!");
    ...
    const cpuBrain = {
      ...
      posPlus1: [
        // CPU is losing (Wall on CPU side). More aggressive with Supers to stay alive.
        // P1 Energy: 0,      1,      2,      3,      4,      5,      6+
        [990000, 990000, 990000, 990000, 990000, 990000, 990000], // CPU 0: Must Rest
        [990000, 990000, 990000, 950400, 900900, 801900, 702900], // CPU 1: Mostly Rest, probe if P1 is high
        [990000, 990000, 900900, 801900, 702900, 603900, 405900], // CPU 2: Resting to 5
        [851400, 801900, 752400, 603900, 405900, 207900, 108900], // CPU 3: One Rest away from Gold! Push if P1 is dangerous.
        [200574, 201069, 202059, 153054, 104049, 056034, 019008], // CPU 4: Has Silver. High chance to spoil P1's Rest.
        [050193, 050193, 050193, 050589, 051084, 052074, 014058], // CPU 5: High greed for Gold, but will Silver Super if P1 is at 5+.
        [000099, 000099, 000099, 000099, 000099, 000099, 000099]  // CPU 6+: Fire the Gold.
      ],

      posZero: [
        // Neutral Ground. Calculated efficiency.
        // P1 Energy: 0,      1,      2,      3,      4,      5,      6+
        [990000, 990000, 990000, 990000, 990000, 990000, 990000], // CPU 0: Must Rest
        [990000, 990000, 990000, 990000, 950400, 900900, 801900], // CPU 1: Greedier for rest
        [990000, 990000, 950400, 900900, 801900, 702900, 603900], // CPU 2: Rest to 5
        [900900, 851400, 801900, 702900, 504900, 306900, 108900], // CPU 3: Pushing more to stop P1 from hitting 6
        [300564, 301059, 251559, 202059, 153054, 104049, 059400], // CPU 4: Bluffing/Spoiling
        [150183, 150183, 100188, 100584, 051084, 052074, 019800], // CPU 5: One step from Gold.
        [000099, 000099, 000099, 000099, 000099, 000099, 000099]  // CPU 6+: Gold.
      ],

      posMinus1: [
        // CPU is winning. Logic updated to "Gatekeep" Player 1 from reaching Gold.
        // P1 Energy: 0,      1,      2,      3,      4,      5,      6+
        [990000, 990000, 990000, 990000, 990000, 990000, 990000], // CPU 0: Must Rest

        // CPU 1: Now pushes 60-80% of the time if P1 is at 3-5 energy
        [990000, 990000, 801900, 306900, 207900, 108900, 059400],

        // CPU 2: Increasing push frequency to stop the P1 "Rest-to-6"
        [990000, 950400, 851400, 405900, 306900, 207900, 108900],

        // CPU 3: High pressure. CPU will sacrifice its own "Gold" progress to stop yours.
        [900900, 801900, 702900, 504900, 405900, 306900, 207900],

        [400158, 400554, 351054, 301554, 252054, 154044, 108900], // CPU 4
        [200178, 200178, 200178, 200178, 151074, 102069, 059400], // CPU 5
        [000099, 000099, 000099, 000099, 000099, 000099, 000099]  // CPU 6+
      ]
    };
